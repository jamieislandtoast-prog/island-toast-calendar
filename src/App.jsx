...
const LINE_URL = `https://line.me/R/ti/p/${FIXED_LINE_ID.replace("@", "~")}`;
...
// Firestore Paths（events 不動；settings / menuNews）
const PATHS = {
  eventsCol: () => collection(db, "artifacts", ISLAND_APP_ID, "public", "data", "events"),
  settingsDoc: () => doc(db, "artifacts", ISLAND_APP_ID, "public", "data", "settings", "general"),
  menuNewsCol: () => collection(db, "artifacts", ISLAND_APP_ID, "public", "data", "menuNews"),
};
...
export default function App() {
  ...
  const [appSettings, setAppSettings] = useState({
    ...
    logoUrl: "https://images.unsplash.com/photo-1509440159596-0249088772ff?auto=format&fit=crop&w=200",
  });
  ...
  const [editingEvent, setEditingEvent] = useState({ location: "", address: "", time: "", type: "work" });
  ...
  useEffect(() => {
    ...
    const runLogin = async () => {
      try {
        // 已登入就不重登
        unsubAuth = onAuthStateChanged(auth, (u) => setUser(u || null));
    ...
    if ("requestIdleCallback" in window) {
      window.requestIdleCallback(() => runLogin(), { timeout: 1200 });
    } else {
      setTimeout(() => runLogin(), 150);
    }
  ...
  // ================
  // MENU & NEWS 同步（最多拿 20，但 UI 只顯示 5）
  // ================
  useEffect(() => {
    if (!user) return;

    const q = query(PATHS.menuNewsCol(), orderBy("createdAt", "desc"), limit(20));
    const unsub = onSnapshot(
      q,
      (snap) => {
        const data = [];
      ...
      (err) => console.error("Firestore menuNews error:", err)
    );
  ...
  const editSettingField = async (field, label) => {
    ...
    const next = window.prompt(`編輯${label}\n（副標題可用 Enter 換行）`, current);
    if (next === null) return;
    ...
    try {
      ...
      console.error(e);
      alert("更新失敗：請確認 Firestore Rules 是否允許匿名寫入。");
    } finally {
  ...
  const handleLogoUpload = async (e) => {
    ...
    setIsUploading(true);
    try {
      const base64 = await compressImage(file, 520, 0.82);
      await setDoc(PATHS.settingsDoc(), { logoUrl: base64 }, { merge: true });
    } catch (err) {
      console.error(err);
      alert("Logo 上傳失敗：請檢查網路、圖片格式、或 Firestore Rules/大小限制。");
    } finally {
      setIsUploading(false);
      e.target.value = "";
    }
  ...
  const handleMenuNewsUpload = async (e) => {
    ...
    const currentCount = Math.min(menuNews.length, 5);
    const remain = Math.max(0, 5 - currentCount);
    if (remain <= 0) {
    ...
    try {
      ...
      console.error(err);
      alert("MENU & NEWS 上傳失敗：請檢查 Firestore Rules/網路/圖片大小。");
    } finally {
      setIsUploading(false);
      e.target.value = "";
    }
  ...
  const deleteMenuNews = async (id) => {
    ...
    setIsUploading(true);
    try {
      await deleteDoc(doc(db, "artifacts", ISLAND_APP_ID, "public", "data", "menuNews", id));
    } catch (err) {
      console.error(err);
    ...
    for (let r = 0; r < totalRows; r++) {
      ...
      for (let c = 0; c < 7; c++) {
        ...
        if (dayNum > 0 && dayNum <= daysInMonth) {
          ...
          cells.push(
            <div
              ...
              onClick={() => {
                ...
                setEditingEvent(ev || { location: "", address: "", time: "", type: "work" });
                setIsModalOpen(true);
              }}
          ...
          cells.push(<div key={`e-${cellIdx}`} className="h-16 border-t border-r border-gray-100 bg-gray-50/20" />);
        }
      }
          ...
          return (
            <div
              ...
              className={`flex items-center gap-4 p-4 rounded-2xl border transition-all cursor-pointer ${
                isToday ? "border-orange-500 bg-orange-50/30" : "border-gray-100 bg-white hover:border-orange-200"
              }`}
            >
                    ...
                    {ev.type === "work" && (
                      <div className="flex flex-col gap-0.5 mt-1">
                        <div className="text-[10px] text-orange-600 font-bold flex items-center gap-1">
                          <Clock size={12} /> {ev.time || "時間待定"}
                        </div>
                        <div className="text-[10px] text-gray-400 truncate flex items-center gap-1">
                          <MapPin size={12} /> {ev.address || "地點詳洽"}
                        </div>
                      </div>
                    )}
                  </div>
  ...
  const saveEvent = async () => {
    ...
    setIsUploading(true);
    try {
      await setDoc(doc(db, "artifacts", ISLAND_APP_ID, "public", "data", "events", selectedDate), editingEvent);
    ...
    <div className="min-h-screen py-6 px-4 flex flex-col items-center">
      <div className="w-full max-w-md bg-white shadow-2xl rounded-[3rem] overflow-hidden relative border border-gray-100">
        ...
        <div className="pt-14 pb-8 flex flex-col items-center">
          {/* Logo（管理員可點擊更換） */}
          <div className={`relative ${isAdmin ? "cursor-pointer" : ""}`} onClick={onPickLogo} title={isAdmin ? "點擊更換 Logo" : ""}>
            <img
          ...
          <div className="bg-white rounded-[2rem] border border-gray-100 overflow-hidden shadow-sm">
            <div className="flex items-center justify-between p-5 bg-orange-50/50">
              <button
                ...
                className="p-1.5 bg-white rounded-full shadow-sm no-print"
              >
            ...
            {viewMode === "month" ? (
              <>
                <div className="calendar-grid text-center border-t border-gray-100 bg-gray-50/30">
                  <div className="py-2.5 text-[9px] font-black text-gray-300 border-r border-gray-100">WEEK</div>
                  {["日", "一", "二", "三", "四", "五", "六"].map((d) => (
                    <div key={d} className="py-2.5 text-[10px] font-bold text-gray-400 border-r border-gray-100 last:border-0">
                      {d}
                    </div>
                  ))}
                </div>
        ...
        <div className="px-8 pb-10">
          ...
          >
            <MessageSquare color="white" /> LINE 預訂行程
          </a>
        </div>
        ...
        <div className="px-8 pb-12">
          <div className="flex items-center justify-between mb-3">
            <div className="flex flex-col">
              <div className="text-[10px] font-black text-orange-500 tracking-widest uppercase">Menu & News</div>
            ...
            {isAdmin && (
              <button
                ...
                className="no-print px-4 py-2 rounded-full bg-black text-white text-[10px] font-black active:scale-95 transition-transform flex items-center gap-2"
                title="最多 5 張"
              >
                <ImagePlus size={14} /> 新增（最多5）
              </button>
            )}
          </div>
          ...
          {isAdmin && (
            <div className="no-print mt-3 text-[10px] text-gray-400 font-bold">
              目前：{Math.min(menuNews.length, 5)} / 5（超過會自動只顯示前 5）
            </div>
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-8">
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-orange-500 tracking-widest uppercase">Schedule Detail</span>
                  ...
                  <div className="space-y-6">
                    ...
                    >
                      <div className="text-2xl font-black text-gray-800 leading-tight mb-2">
                        {events[selectedDate].type === "rest" ? "今日店休 REST DAY" : events[selectedDate].location}
                      </div>

                      {events[selectedDate].type === "work" && (
                        <div className="space-y-3 mt-4">
                          <div className="text-orange-600 font-black flex items-center justify-center gap-2">
                            <Clock size={16} /> {events[selectedDate].time || "14:00 - 完售"}
                          </div>

                          {events[selectedDate].address && (
                            <div className="text-gray-400 text-sm font-bold flex items-center justify-center gap-2 leading-relaxed">
                              <MapPin size={16} /> {events[selectedDate].address}
                            </div>
                          )}
                        </div>
                      )}
                    ...
                    {events[selectedDate].type === "work" && (
                      ...
                      >
                        <MapIcon color="white" /> Google 地圖導航
                      </a>
                    )}
                  </div>
                ) : (
